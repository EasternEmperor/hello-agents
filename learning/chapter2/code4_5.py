...
from code4_4 import HelloAgentsLLM, PlanAndSolveAgent

class Memory:
    """
    一个简单的短期记忆模块，用于存储智能体行动和反思轨迹
    """
    ...
    def get_trajectory(self) -> str:
        """
        将所有记忆记录格式化为一个连贯的字符串文本并返回，用于构建提示词
        """
        trajectory_parts = []
...
如果代码在算法层面已经达到最优，则只需要输出“无需改进”四个字即可，不需要标点符号。

请直接输出你的反馈，不要包含任何额外的解释。
"""
REFINE_PROMPT_TEMPLATE2 = """
你是一位注重代码可读性的开源项目维护者，对代码的性能有极致的要求。
你的任务是审查以下Python代码，并专注于找出其在<strong>算法效率</strong>上的主要瓶颈。
...
class ReflectionAgent:
    ...
    def run(self, task: str):
        ...
        for i in range(self.max_iterations):
            ...
            # reflect_prompt = REFINE_PROMPT_TEMPLATE2.format(task=task, code=last_code)
